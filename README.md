...
- 圓形旋鈕控制器，支持鼠標和觸控操作
- 可自定義最小值、最大值、顏色、尺寸等屬性
- 支持v-model雙向綁定（包括值和背景顏色）
- 支持自定義起始角度和總角度範圍
- 支持小數點顯示格式化
- 支持步進值設定，控制數值變化的精度
- 支持自定義內環背景顏色
...
- 使用SVG路徑(`<path>`)繪製進度環，而不是簡單的圓形，提供更精確的視覺效果
- 採用多段弧形繪製技術，避免在50%處的視覺跳變問題
- 優化的拖動處理邏輯，支持整個元件區域的拖動操作
- 精確的角度計算，確保在任何起始角度下都能正確工作
- 拖動點具有懸停和激活效果，提供更好的視覺反饋

## 線上演示

您可以訪問以下網址查看元件的在線演示：

[Vue3 Knob 元件演示](https://w-bgmotion-knob.deno.dev/)

該演示網站展示了 Knob 元件的各種功能和配置選項，您可以直接體驗元件的交互效果和視覺表現。
...
演示頁面提供互動式界面，可以實時調整所有參數並查看其對Knob元件的影響：

- 調整當前值、最小值、最大值
- 設置步進值，控制數值變化的精度
- 自定義小數位數顯示
- 調整尺寸、起始角度和總角度範圍
- 自定義進度環、背景環、文字和內環背景的顏色

## Knob元件屬性

| 屬性 | 類型 | 默認值 | 說明 |
|------|------|--------|------|
| modelValue | Number | - | 當前值，支持v-model綁定 |
| min | Number | 0 | 最小值 |
| max | Number | 100 | 最大值 |
...
| backgroundColor | String | 'transparent' | 內環背景顏色，支持v-model:backgroundColor綁定 |
